"""src.models.transformer_model.TransformerModel 的单元测试。

风格镜像 test_lstm_model.py：7 个核心测试 + 2 个额外验证。
单测用最小超参（hidden_dim=8, nhead=2, num_layers=1, epochs=2）让 CPU 也能秒过。
...
def cleaned_df() -> pd.DataFrame:
    """复用真实清洗数据，整个模块共享（与 test_lstm_model.py 一致）。"""
...
def _make_features(
    ...
    """从 cleaned_df 切片，构造 Transformer 训练特征（用真实 Appliances 列）。"""
    sample = df.iloc[:n]
...
def test_transformer_fit_returns_self(cleaned_df):
...
def test_transformer_predict_shape_and_positive(cleaned_df):
    ...
    assert (preds > 0).all(), "Transformer 预测必须为正（expm1 反变换）"


def test_transformer_predict_before_fit_raises():
    """未 fit 就 predict 应抛 RuntimeError（与 LSTMModel 镜像）。"""
...
def test_transformer_save_load_roundtrip(tmp_path, cleaned_df):
    ...
    # 保存
    p = tmp_path / "transformer.pt"
    model.save(p)
    assert p.exists()

    # 加载 + 推理
    loaded = TransformerModel.load(p)
    preds_after = loaded.predict(X_pred)
    np.testing.assert_allclose(preds_before, preds_after, rtol=1e-5)


def test_transformer_feature_names_aligned(cleaned_df):
    """用错列名/列序的 X 应抛 ValueError（防线上推理事故，镜像 LSTMModel）。"""
    params = _tiny_params()
    model = TransformerModel(
        params=params,
...
def test_transformer_fit_empty_x_raises():
...
def test_transformer_fit_length_mismatch_raises():
...
def test_transformer_save_before_fit_raises(tmp_path):
    """未 fit 就 save 应抛 RuntimeError（防线上事故）。"""
    model = TransformerModel()
    p = tmp_path / "transformer_unfit.pt"
...
def test_transformer_fit_uses_cuda_if_available(caplog, cleaned_df):
    """fit 应自动选 cuda/cpu（云端 GPU 训练前提）。"""
    import logging
    caplog.set_level(logging.INFO, logger="src.models.transformer_model")
    ...
    # 验证日志至少有一条 epoch 信息
    assert any("Transformer epoch" in rec.message for rec in caplog.records), (
        "应至少有一条 Transformer epoch 训练日志"
    )
